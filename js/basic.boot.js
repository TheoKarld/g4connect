...
function fetchvalu(o){
	var r={};
	for(var i in o)r[i]=(o[i].type=='number'||o[i].type=='email')?o[i].value:cleaname(o[i].value);
	return r;
...
var myEditor=function(element,father){
	...
	addEvent(VP,'mouseover',function(e){
		e=ee(e);
		showid(e);
	});
...
var flexie=function(id,ct,bt,ag){
	...
	function flipme(t){
		if(!so){
			ip.style.display='none';if(b2)b2.style.display='none';sp.style.display='';if(b1)b1.style.display='';
...
var ulist=function(id,rc,lc,ar){
	var rd=$$$('ul',id,rc),li,eo={},c,ac;
	eo.u=rd;eo.l={};
	if(ar)for(var i in ar){
		c=(ar[i]==id)?lc.s:lc.d;
		ac=anc('#',ar[i])
		li=$$$('li','',c,'',[ac]);
		if(ar[i]==id)ac.style.color='white';
		eo.l[ar[i]]=li
		APP(rd,li);
	}
	return eo;
...
var dayat=function(d){
	var a=(typeof d=='string')?d:d.toString(),ar=['st','nd','rd','th','nth'],v=parseInt(a.charAt(ocn(a)-1));
	if(v==1&&d!=11)a=a+ar[0];
	if(v==2&&d!=12)a=a+ar[1];
	if(v==3&&d!=13)a=a+ar[2];
	if(v==4||v==5||v==6||v==8||(v==0&&ocn(a)>1)||(d>10&&d<20))a=a+ar[3];
	if(v==7&&d!=17||v==9&&d!=19)a=a+ar[4];
	return a;
}
var dates=function(v){
	var a=v.split('-'),ds=dayat(parseInt(a[2]))+' '+mns[parseInt(a[1])-1]+' '+a[0];
	return ds;
}
var myul=function(id,ar,ic){
	var rd=pah(id),p,a,eo={};
	eo.e=rd;eo.o={};
	for(var i in ar){
		p=par([icon(ic[i]),anc('#',ar[i])],'myul-p');
		eo.o[ar[i]]=p;
		APP(rd,p);
	}
	return eo;
...
var sieul=function(a,b,g){
	var rd=jum('ieul'),eo={},c,h=cta(a),k;
	eo.e=rd;
	eo.o={};
	for(var i in h){
		k=(g&&g[i])?[g[i]]:h[i];
		c=flexie(h[i],{t:k,v:b[h[i]]});
		APP(rd,c.e);
		eo.o[h[i]]=c;
	}
